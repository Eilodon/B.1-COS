...
use pandora_cwm::gnn::types::{GnnConfig, CausalEdgeKind, CausalEdge};
...
fn test_active_inference_planning_basic() {
    ...
    assert!(flow.sankhara.is_some(), "Sankhara should have formed an intent");
    
    let intent = flow.sankhara.as_ref().unwrap();
    assert!(
        ["action_A", "action_B", "action_C", "default_fallback_intent"].contains(&intent.as_ref()),
...
fn test_planning_with_different_horizons() {
    ...
    for horizon in [1, 3, 5] {
        ...
        assert!(flow.sankhara.is_some(), "Sankhara should form intent with horizon {}", horizon);
    }
}
...
fn test_planning_with_context_specific_actions() {
    ...
    assert!(flow.sankhara.is_some(), "Sankhara should form intent based on context");
}
...
fn test_planning_error_handling() {
    ...
    assert!(flow.sankhara.is_some(), "Sankhara should always form some intent");
...
fn test_complete_planning_workflow() {
    ...
    cwm.gnn_mut().add_edge(state_node, goal_node, positive_edge).unwrap();
    cwm.gnn_mut().add_edge(obstacle_node, goal_node, negative_edge).unwrap();
    ...
    let mut flow = EpistemologicalFlow::from_bytes(Bytes::from(b"agent_needs_to_navigate".as_ref()));
    ...
    let intent = flow.sankhara.as_ref().unwrap();
    assert!(
        ["move_toward_goal", "avoid_obstacle", "explore", "default_fallback_intent"].contains(&intent.as_ref()),
...
fn test_planning_with_different_reward_configs() {
    ...
    for (exploit_weight, transcend_weight) in reward_configs {
        ...
        assert!(flow.sankhara.is_some(), "Should form intent with weights ({}, {})", exploit_weight, transcend_weight);
    }
}
