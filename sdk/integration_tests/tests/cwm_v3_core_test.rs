...
use pandora_cwm::gnn::types::{CausalEdgeKind, CausalEdge, GnnConfig};
...
fn test_cwm_v3_model_instantiation() {
    ...
    assert_eq!(model.gnn().edge_count(), 0, "Initial graph should have no edges");
}
...
fn test_causal_relationship_creation() {
    ...
    assert!(edge1_result.is_ok(), "Precondition edge should be added successfully");
    assert!(edge2_result.is_ok(), "Cause edge should be added successfully");
...
fn test_edge_type_default_weights() {
    ...
    for edge_kind in edge_types {
        ...
        assert!(result.is_ok(), "Edge with default weight should be added successfully");
    }
    
    assert_eq!(model.gnn().edge_count(), 5, "Should have 5 edges with different types");
}
...
fn test_error_handling() {
    ...
    assert!(result.is_err(), "Adding edge to non-existent node should fail");
}
...
fn test_config_validation() {
    ...
    let invalid_configs = vec![
        GnnConfig { input_dims: 0, hidden_dims: 128, num_layers: 3, learning_rate: 0.001, dropout_rate: 0.1, use_attention: true },
        GnnConfig { input_dims: 64, hidden_dims: 0, num_layers: 3, learning_rate: 0.001, dropout_rate: 0.1, use_attention: true },
        GnnConfig { input_dims: 64, hidden_dims: 128, num_layers: 0, learning_rate: 0.001, dropout_rate: 0.1, use_attention: true },
        GnnConfig { input_dims: 64, hidden_dims: 128, num_layers: 3, learning_rate: 0.0, dropout_rate: 0.1, use_attention: true },
        GnnConfig { input_dims: 64, hidden_dims: 128, num_layers: 3, learning_rate: 0.001, dropout_rate: 1.0, use_attention: true },
    ];
...
fn test_edge_metadata() {
    ...
    let result = model.gnn_mut().add_edge(node_a, node_b, edge);
    assert!(result.is_ok(), "Edge with metadata should be added successfully");
    ...
    assert_eq!(edge_data.metadata, Some("Test causal relationship".to_string()));
}
