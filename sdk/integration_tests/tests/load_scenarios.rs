...
async fn test_realistic_web_service_load() {
    ...
    println!("🚀 Starting realistic web service load test for {:?}", duration);
    ...
    while start.elapsed() < duration {
        ...
        let (skill, input) = if rand < 70 {
            ...
            ("arithmetic", json!({"expression": format!("{} + {}", a, b)}))
        } else if rand < 90 {
            // 20% pattern matching
    ...
    println!("  Requests/sec: {:.2}", total_requests as f64 / duration.as_secs_f64());
...
async fn test_burst_traffic_scenario() {
    ...
    println!("  Throughput: {:.2} req/sec", 1000.0 / elapsed.as_secs_f64());
    
    // We expect most requests to succeed even during burst
    assert!(successes > 900, "Too many failures during burst: {}", 1000 - successes);
}
