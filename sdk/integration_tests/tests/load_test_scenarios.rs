...
async fn test_orchestrator_concurrent_load() {
    ...
    info!("Starting load test: {} users, {} requests each = {} total", 
    ...
    info!("Successes: {} ({:.2}%)", total_stats.successes, total_stats.success_rate);
    info!("Errors: {} ({:.2}%)", total_stats.errors, total_stats.error_rate);
    ...
    if total_stats.success_rate < 30.0 {
        warn!("Very low success rate: {:.2}% - this may indicate system issues", total_stats.success_rate);
    }
    assert!(total_stats.success_rate > 20.0, "Success rate too low: {:.2}%", total_stats.success_rate);
    assert!(total_stats.avg_latency < Duration::from_millis(2000), "Average latency too high: {:?}", total_stats.avg_latency);
...
async fn test_orchestrator_payload_distribution() {
    ...
    for (size_name, size) in payload_sizes {
        ...
        info!("{} payload: {:.1}% success, avg latency: {:?}", 
              size_name, success_rate, avg_latency);
        
        assert!(success_rate > 90.0, "Success rate too low for {} payload", size_name);
        assert!(avg_latency < Duration::from_millis(500), "Latency too high for {} payload", size_name);
    }
}
...
async fn test_orchestrator_circuit_breaker_load() {
    ...
    impl pandora_core::interfaces::skills::SkillModule for FlakySkill {
            ...
            -> pandora_core::interfaces::skills::SkillOutput 
        {
            ...
            if random < self.failure_rate {
                Err(pandora_error::PandoraError::skill_exec("flaky_skill", "Random failure"))
            } else {
    ...
    info!("Circuit breaker load test: {} successes, {} failures", successes, circuit_opens);
...
async fn test_orchestrator_memory_pressure() {
    ...
    info!("Memory pressure test: {:.1}% success, avg latency: {:?}", 
          success_rate, avg_latency);
    
    assert!(success_rate > 90.0, "Success rate too low under memory pressure");
    assert!(avg_latency < Duration::from_millis(200), "Latency too high under memory pressure");
}
