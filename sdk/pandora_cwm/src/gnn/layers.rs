...
use crate::gnn::types::{CausalGraph, CausalEdgeKind};
...
/// ```
    pub fn forward(&self, graph: &CausalGraph, node_features: &Array2<f32>) -> Result<Array2<f32>, PandoraError> {
    ...
    fn compute_attention_score(
        ...
        let edge_weights = self.edge_type_weights.get(&edge_kind)
...
mod tests {
    ...
    fn test_attention_score_computation() {
        ...
        let score = layer.compute_attention_score(&src_features, &tgt_features, CausalEdgeKind::Cause);
        assert!(score.is_ok());
