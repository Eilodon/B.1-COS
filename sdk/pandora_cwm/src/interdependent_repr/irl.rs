...
impl InterdependentNetwork {
    ...
    fn calculate_influence(&self, from: &str, to: &str) -> f64 {
        ...
        let direct_weight = self.relationships.get(&(from.to_string(), to.to_string()))
            .copied()
            .unwrap_or(0.0);
        ...
        for (intermediate, _) in &self.entities {
            if let Some(weight1) = self.relationships.get(&(from.to_string(), intermediate.clone())) {
                if let Some(weight2) = self.relationships.get(&(intermediate.clone(), to.to_string())) {
    ...
    pub fn find_key_influencers(&self, entity_id: &str) -> Vec<(String, f64)> {
        ...
        for (other_id, _) in &self.entities {
            if other_id != entity_id {
                ...
                if influence > 0.1 { // Ngưỡng ảnh hưởng tối thiểu
