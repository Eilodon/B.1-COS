...
#[cfg(test)]
use crate::{ValueDrivenPolicy, NeuralQValueEstimator, LearningEngine, ActiveInferenceSankharaSkandha};
#[cfg(test)]
...
fn test_multi_step_planning_complex_scenario() {
    ...
    let available_actions = vec![
        "move_to_room_a", "move_to_room_b", "move_to_room_c",
        "pick_up_key", "pick_up_distractor", "open_box",
        "check_room_contents", "examine_item", "use_key",
        "wait", "explore", "plan_path"
    ];
    ...
    println!("   Final state: Room = {}, Has key = {}, Box opened = {}", 
...
fn test_dynamic_obstacle_planning() {
    ...
    let available_actions = vec![
        ...
        "jump_over", "climb_around", "wait_for_clear", "find_alternative",
        "assess_obstacle", "plan_detour", "retreat", "persist"
    ];
    ...
    for step in 1..=15 {
        ...
        let action = flow.sankhara.as_ref()
            .map(|s| s.as_ref())
            .unwrap_or("wait");
...
fn test_planning_efficiency() {
    ...
    println!("   Cycles per second: {:.2}", 100.0 / total_time.as_secs_f64());
    
    // Performance assertions
    assert!(total_time.as_millis() < 1000, "Planning should complete within 1 second");
    assert!(avg_time_per_cycle.as_millis() < 10, "Each cycle should take less than 10ms");
