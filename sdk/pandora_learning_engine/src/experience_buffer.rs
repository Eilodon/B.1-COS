...
impl ExperienceBuffer {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &ExperienceSample> { self.samples.iter() }
}
...
mod tests {
    ...
    fn buffer_respects_capacity() {
        ...
        buf.push(ExperienceSample { flow: flow.clone(), reward: 1.0 });
        buf.push(ExperienceSample { flow: flow.clone(), reward: 2.0 });
    ...
    fn priority_sampling_prefers_higher_priority() {
        ...
        buf.push(ExperienceSample { flow: mk("a"), reward: 0.0 }, 1.0);
        buf.push(ExperienceSample { flow: mk("b"), reward: 0.0 }, 10.0);
        buf.push(ExperienceSample { flow: mk("c"), reward: 0.0 }, 1.0);

        // sample multiple seeds, expect index 1 to appear often
        let mut hits_mid = 0;
        for k in 0..100 {
            if let Some(i) = buf.sample_index(k) { if i == 1 { hits_mid += 1; } }
        }
        assert!(hits_mid > 40);
    }
}
