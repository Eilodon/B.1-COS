...
#[cfg(test)]
use crate::{ValueDrivenPolicy, Action, NeuralQValueEstimator, LearningEngine, ActiveInferenceSankharaSkandha};
#[cfg(test)]
...
fn test_non_attachment_learning_environment_change() {
    ...
    for cycle in 1..=200 {
        ...
        if cycle % 25 == 0 {
            println!("\n🔄 Cycle {} ({}): Testing adaptation", cycle, environment_phase);
        }
        
        // Let the system plan and select action
        sankhara.form_intent(&mut flow);
        
        let selected_action = flow.sankhara.as_ref()
            .map(|s| s.as_ref())
            .unwrap_or("action_a");
        
        // Count action selections
        *action_counts.entry(selected_action.to_string()).or_insert(0) += 1;
        ...
        if cycle % 10 == 0 {
            ...
            let action_a_ratio = recent_actions.iter().filter(|&&a| a == "action_a").count() as f64 / 10.0;
            let action_b_ratio = recent_actions.iter().filter(|&&a| a == "action_b").count() as f64 / 10.0;
            
            adaptation_metrics.push((cycle, action_a_ratio, action_b_ratio));
            
            if cycle % 25 == 0 {
                println!("   Recent action distribution: A={:.1}%, B={:.1}%", 
                         action_a_ratio * 100.0, action_b_ratio * 100.0);
            }
        }
        ...
        if cycle == 150 {
            ...
            println!("   📊 Mid-adaptation: Action B ratio = {:.1}%", recent_b_ratio * 100.0);
        }
    }
    ...
    let phase_1_a_ratio = phase_1_actions.iter().filter(|&&a| a == "action_a").count() as f64 / phase_1_actions.len() as f64;
    let phase_2_b_ratio = phase_2_actions.iter().filter(|&&a| a == "action_b").count() as f64 / phase_2_actions.len() as f64;
    
    println!("   Phase 1 (cycles 1-100): Action A ratio = {:.1}%", phase_1_a_ratio * 100.0);
    println!("   Phase 2 (cycles 101-200): Action B ratio = {:.1}%", phase_2_b_ratio * 100.0);
    ...
    println!("   Adaptation speed: {} cycles to reach 50% Action B", adaptation_cycles);
    ...
    assert!(phase_1_a_ratio > 0.3, "Agent should prefer Action A in Phase 1");
    assert!(phase_2_b_ratio > 0.3, "Agent should adapt to Action B in Phase 2");
    assert!(phase_2_avg_reward > 0.5, "Agent should maintain good performance in Phase 2");
    assert!(adaptation_cycles < 150, "Agent should adapt within 50 cycles of environment change");
...
fn test_gradual_environment_change() {
    ...
    let available_actions = vec!["conservative_action", "moderate_action", "aggressive_action"];
    ...
    for cycle in 1..=150 {
        ...
        let reward = if cycle <= 50 {
            // Conservative phase
            if selected_action == "conservative_action" { 1.0 } else { 0.3 }
        } else if cycle <= 100 {
            ...
            // Aggressive phase
            if selected_action == "aggressive_action" { 1.0 } else { 0.3 }
        };
        ...
        if cycle % 25 == 0 {
            ...
            println!("   Cycle {}: Recent avg reward = {:.3}", cycle, recent_reward);
        }
    }
    ...
    println!("   Early performance (cycles 1-50): {:.3}", early_performance);
    println!("   Late performance (cycles 101-150): {:.3}", late_performance);
    println!("   Performance maintained: {}", late_performance > 0.7);
    
    assert!(late_performance > 0.7, "Agent should maintain good performance after gradual change");
...
fn test_catastrophic_environment_change() {
    ...
    for cycle in 1..=200 {
        ...
        let reward = if cycle <= 100 {
            ...
            if selected_action == "old_strategy" { 1.0 } else { 0.2 }
        } else {
            // After change: old strategy is catastrophic
            if selected_action == "old_strategy" { -1.0 }
            else if selected_action == "new_strategy" { 0.8 }
            else { 0.1 }
        };
        ...
        if cycle % 25 == 0 && cycle > 100 {
            ...
            println!("   Cycle {}: Recent avg reward = {:.3}, Old strategy usage = {:.1}%", 
                     cycle, recent_reward, old_strategy_usage * 100.0);
        }
    }
    ...
    println!("   Final old strategy usage: {:.1}%", final_old_strategy_usage * 100.0);
    println!("   Successfully abandoned old strategy: {}", final_old_strategy_usage < 0.2);
    
    assert!(post_change_performance > 0.0, "Agent should avoid catastrophic negative rewards");
    assert!(final_old_strategy_usage < 0.3, "Agent should largely abandon the old strategy");
