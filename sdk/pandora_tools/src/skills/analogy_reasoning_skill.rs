...
impl SkillModule for AnalogyReasoningSkill {
    ...
    async fn execute(&self, input: SkillInput) -> SkillOutput {
        let a = input
            .get("a")
            .and_then(|v| v.as_str())
            .ok_or(PandoraError::InvalidSkillInput {
                skill_name: "analogy_reasoning".into(),
                message: "Missing field 'a'".into(),
            })?;
        let b = input
            .get("b")
            .and_then(|v| v.as_str())
        ...
        let c = input
            ...
            .ok_or(PandoraError::InvalidSkillInput {
                skill_name: "analogy_reasoning".into(),
                message: "Missing field 'c'".into(),
            })?;
        let candidates = input.get("candidates").and_then(|v| v.as_array()).ok_or(
            PandoraError::InvalidSkillInput {
                skill_name: "analogy_reasoning".into(),
                message: "Missing field 'candidates'".into(),
            },
        )?;
        ...
        if let Some(ans) = best {
            ...
            Err(PandoraError::skill_exec(
                "analogy_reasoning",
                "Không tìm được đáp án phù hợp",
            ))
        }
    }
}
