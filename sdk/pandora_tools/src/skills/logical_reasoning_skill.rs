...
impl SkillModule for LogicalReasoningSkill {
	...
	async fn execute(&self, input: SkillInput) -> SkillOutput {
        let ast = input.get("ast").ok_or_else(|| PandoraError::InvalidSkillInput { skill_name: "logical_reasoning".into(), message: "Missing 'ast' field".into() })?;
        let context = input.get("context").and_then(|v| v.as_object()).ok_or_else(|| PandoraError::InvalidSkillInput { skill_name: "logical_reasoning".into(), message: "Missing or invalid 'context' field".into() })?;
