...
impl SkillModule for LogicalReasoningSkill {
    ...
    async fn execute(&self, input: SkillInput) -> SkillOutput {
        let ast = input.get("ast").ok_or(PandoraError::SkillExecution {
            skill_name: "logical_reasoning".into(),
            message: "Thiếu trường 'ast'".into(),
        })?;
        let context = input.get("context").and_then(|v| v.as_object()).ok_or(
            PandoraError::SkillExecution {
                skill_name: "logical_reasoning".into(),
                message: "Thiếu hoặc sai kiểu 'context'".into(),
            },
        )?;
